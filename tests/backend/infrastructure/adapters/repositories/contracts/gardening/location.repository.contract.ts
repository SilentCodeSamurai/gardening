...
export function registerLocationRepositoryContract(
	...
	describe(`LocationRepositoryPort (${adapterLabel})`, () => {
		let location: ReturnType<typeof resolveGardeningRepositoryPorts>["location"];
		...
		it("deleteMany count 0 when no matches", async () => {
			const { count } = await location.deleteMany({
				filters: [{ name: "no-such-location-name", workspace: wk }],
			});
			expect(count).toBe(0);
		});
		...
		it("updateMany OR patches every matched row; count 0 when nothing matches", async () => {
			...
			const z = await location.updateMany({
				filters: [{ name: "no-such-loc-999", workspace: wk }],
				dto: { name: "noop" },
			});
