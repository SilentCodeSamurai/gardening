...
export const Route = createFileRoute("/catalog/species")({
...
function SpeciesPage() {
	...
	const categoryTitle = useMemo(() => {
		...
		for (const c of catData?.items ?? []) {
			map.set(String(c.id), translateCatalogField(c.title, c.isDefault) ?? String(c.id));
		}
		return map;
	}, [catData?.items]);
	...
	const columnHelper = useMemo(() => createColumnHelper<SpeciesEntity>(), []);
	const columns = useMemo(
		() => [
			...
			columnHelper.accessor(
				(s) => {
					const name = translateCatalogField(s.characteristics.name, s.isDefault) ?? "";
					const desc = translateCatalogField(s.characteristics.description, s.isDefault) ?? "";
			...
			columnHelper.accessor((s) => translateCatalogField(s.characteristics.name, s.isDefault) ?? "", {
				id: "name",
			...
			columnHelper.accessor((s) => String(s.categoryId), {
				...
				enableGlobalFilter: false,
				meta: {
					filter: ({ column }: { column: Column<SpeciesEntity, unknown> }) => {
			...
			columnHelper.accessor((s) => translateCatalogField(s.characteristics.description, s.isDefault) ?? "", {
				id: "description",
			...
			columnHelper.accessor((s) => (s.isDefault ? "default" : "custom"), {
				id: "isCustom",
...
function SpeciesRowActions({ species, categoryId }: { species: SpeciesEntity; categoryId: string }) {
	const [editOpen, setEditOpen] = useState(false);
	const [deleteOpen, setDeleteOpen] = useState(false);
	const del = useSpeciesDeleteMutation();
	const name = translateCatalogField(species.characteristics.name, species.isDefault);

	const editTitle = species.isDefault ? m.common_editDisabledDefaultCatalog() : m.common_edit();
	const deleteTitle = species.isDefault ? m.common_editDisabledDefaultCatalog() : m.common_delete();
